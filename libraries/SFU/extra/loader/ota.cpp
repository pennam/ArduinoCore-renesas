...
int verify_header(FILE* update_file) {
  ...
  debug_if(OTA_DBG, "Reading OTA header..");
...
int decompress(FILE* update_file, FILE* target_file, SFUWatchdogResetFuncPointer wdog_feed_func_ptr) {
  ...
  debug_if(OTA_DBG, "Decompressing OTA binary..");
...
int verify_sketch(FILE* file) {
  ...
  debug_if(OTA_DBG, "Veryfing OTA binary..");
  ...
  if (len < 0) {
    debug_if(OTA_DBG, "OTA binary invalid length: %d", len);
    delete[] page_buffer;
    return -1;
  }
  ...
  while (size_read < POST_APPLICATION_ADDR) {
    ...
    if (memmem((const char*)page_buffer, page_size, "SFU version", 11) != NULL) {
      debug_if(OTA_DBG, "OTA binary signature string found\n");
      break;
    }
  }
  if (size_read >= POST_APPLICATION_ADDR) {
    debug_if(OTA_DBG, "OTA binary does not contain SFU, won't flash it\n");
    delete[] page_buffer;
    return -2;
  }
  delete[] page_buffer;
...
int flash(FILE *file, CodeFlashBlockDevice* cf, uint32_t address)
{
  ...
  debug_if(OTA_DBG, "Flashing OTA binary..");
  ...
  while (true) {
    ...
    if (size_read <= 0) {
      debug_if(OTA_DBG, "Error reading %d bytes from OTA binary", size_read);
      break;
    }
    ...
    if (++pages_flashed % 3 == 0) {
      ...
      if (percent_done != percent_done_new) {
        ...
        debug_if(OTA_DBG, "Flashed %3ld%%", percent_done);
      }
    }
  }
